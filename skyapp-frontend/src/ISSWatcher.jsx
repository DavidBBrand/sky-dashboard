  ...
  return (
    <div className="iss-card" style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <p style={{ letterSpacing: '2px', fontSize: '1rem', color: 'var(--text-sub)' }}>
          ORBITAL TRACKER
        </p>
        <div className={isNearby ? "ping-indicator" : ""} 
             style={{ backgroundColor: isNearby ? "#4ade80" : "#555", width: '8px', height: '8px', borderRadius: '50%' }}>
        </div>
      </div>

      <h2 style={{ color: 'var(--text-main)', margin: '10px 0 5px 0' }}>ISS STATION</h2>
      
      <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: isNearby ? "#4ade80" : "var(--text-main)" }}>
        {distance ? `${distance.toFixed(0)} mi` : "Scanning..."}
      </div>
      
      <p style={{ fontSize: '1.5rem', color: 'var(--text-sub)', marginTop: '5px' }}>
